...
const port = 3004//rota diferente para evitar conflitos
...
for(let usuario of listaDeUsuarios){ // percorre objetos, incluindo Array 
...
app.patch("/usuarios/:id",(req, res)=>{
  ...
  return res.status(404).json({message: " Usuário não localizado. Tente novamente!"})
})
...
app.delete("/usuarios/:id", (req, res) => {
  ...
  const usuario = listaDeUsuarios.findIndex((usuario) => usuario.id == idUsuario) // localiza o índice do usuário que possui aquele id 

   if (usuario === -1) {  // -1 quando não encontra o usuário com determinado id, retornando o erro
